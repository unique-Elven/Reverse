...
void SetHwBreakPoint(HANDLE hThread)
{
	...
	ctx.Dr0 = 0x00401053;
	ctx.Dr7 = 0x01;
	SetThreadContext(hThread, &ctx);//由于在win10中在运行程序的设置上下文的结果是不可预测的，所以注意要先挂起线程
}

DWORD WINAPI ThreadProc(_In_ LPVOID lpParameter)//重新开启一个线程来，挂起主线程
{
	HANDLE hThead = OpenThread(THREAD_ALL_ACCESS, TRUE, (DWORD)lpParameter);
	SuspendThread((HANDLE)hThead);//挂起线程
	SetHwBreakPoint(hThead);
	ResumeThread((HANDLE)hThead);//恢复线程
	return 0;
}
...
namespace AheadLib
{
	...
	inline BOOL WINAPI Load()
	{
		...
		HANDLE hTread = CreateThread(NULL, 0, ThreadProc, (LPVOID)GetCurrentThreadId(), 0, NULL);
